...
```
 ...
 --ero ERO ERO ERO
                 Apply erosion with a specified kernel size (3 integers)
 --tfce TFCE TFCE
                 Apply TFCE (Threshold-Free Cluster Enhancement) with H and E parameters
 --tensor_decomp Perform tensor decomposition and return Fractional Anisotropy (FA) image
 --bptf BPTF BPTF
                 Apply BPTF with the specified spatial and temporal smoothing parameters
 --dt {float,double}
                 Data type for the input image (default: float)
 --odt {float,double}
